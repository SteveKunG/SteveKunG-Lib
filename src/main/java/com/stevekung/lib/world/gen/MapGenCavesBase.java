package com.stevekung.lib.world.gen;
...
public class MapGenCavesBase extends MapGenBase
{
                    ...
                    for (int j1 = k2; !flag3 && j1 < k; ++j1)
                    {
                        for (int k1 = i3; !flag3 && k1 < i1; ++k1)
                        {
                            for (int l1 = l + 1; !flag3 && l1 >= l2 - 1; --l1)
                            {
                                if (l1 < 256)
                                {
                                    if (this.isOceanBlock(primer, j1, l1, k1))
                                    {
                                        flag3 = true;
                                    }
                                ...
                                if (d10 * d10 + d8 * d8 < 1.0D)
                                {
                                    for (int j2 = l; j2 > l2; --j2)
                                    {
                                        ...
                                        if (d9 > -0.7D && d10 * d10 + d9 * d9 + d8 * d8 < 1.0D)
                                        {
                                            ...
                                            if (this.isTopBlock(primer, j3, j2, i2))
                                            {
                                                flag1 = true;
                                            }
    ...
    private boolean isOceanBlock(ChunkPrimer primer, int x, int y, int z)
    {
    ...
    private boolean isTopBlock(ChunkPrimer primer, int x, int y, int z)
    {
    ...
    private void digBlock(ChunkPrimer primer, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop, IBlockState state)
    {
